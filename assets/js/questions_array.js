const questions = [{
    questionNumber: 1,
    questionText: "At a party you are most likely to be found…",
    answers: [{
            answerNumber: 1,
            answerText: "Leafing through the host's book collection",
            answerType: "culture"
        },
        {
            answerNumber: 2,
            answerText: "Jumping off the roof in to the pool",
            answerType: "thrill"
        },
        {
            answerNumber: 3,
            answerText: "Hanging out with the pet dog",
            answerType: "wildlife"
        },
        {
            answerNumber: 4,
            answerText: "Telling a funny story to a crowd of people",
            answerType: "people"
        },
        {
            answerNumber: 5,
            answerText: "In the kitchen making 'helpful' cooking suggestions",
            answerType: "food"
        },
        {
            answerNumber: 6,
            answerText: "Lying in the garden looking at the stars",
            answerType: "remote"
        },
    ]
},
{
    questionNumber: 2,
    questionText: "Your ideal weekend would involve…",
    answers: [{
            answerNumber: 1,
            answerText: "Alpaca walking",
            answerType: "wildlife"
        },
        {
            answerNumber: 2,
            answerText: "A day shopping in a big city",
            answerType: "people"
        },
        {
            answerNumber: 3,
            answerText: "Wandering around a museum",
            answerType: "culture"
        },
        {
            answerNumber: 4,
            answerText: "Getting as far away from other human beings as possible",
            answerType: "remote"
        },
        {
            answerNumber: 5,
            answerText: "Mountain biking down Snowdon",
            answerType: "thrill"
        },
        {
            answerNumber: 6,
            answerText: "Trying out a new restaurant",
            answerType: "food"
        },
    ]
},
{
    questionNumber: 3,
    questionText: "Choose a music genre…",
    answers: [{
            answerNumber: 1,
            answerText: "Punk",
            answerType: "thrill"
        },
        {
            answerNumber: 2,
            answerText: "Silence",
            answerType: "remote"
        },
        {
            answerNumber: 3,
            answerText: "Pop",
            answerType: "people"
        },
        {
            answerNumber: 4,
            answerText: "Jazz",
            answerType: "food"
        },
        {
            answerNumber: 5,
            answerText: "Whalesong",
            answerType: "wildlife"
        },
        {
            answerNumber: 6,
            answerText: "Classical",
            answerType: "culture"
        },
    ]
},
{
    questionNumber: 4,
    questionText: "Which breakfast appeals to you the most?",
    answers: [{
            answerNumber: 1,
            answerText: "Sprouted millet and buckwheat toast with avocado and smoked butter",
            answerType: "food"
        },
        {
            answerNumber: 2,
            answerText: "Coffee at an art gallery cafe",
            answerType: "culture"
        },
        {
            answerNumber: 3,
            answerText: "5am surf followed by a trail bar and coffee in your campervan",
            answerType: "thrill"
        },
        {
            answerNumber: 4,
            answerText: "Anything as long as you can share it with your dog",
            answerType: "wildlife"
        },
        {
            answerNumber: 5,
            answerText: "Toast cooked on a fork over an open fire",
            answerType: "remote"
        },
        {
            answerNumber: 6,
            answerText: "Brunch out in a busy cafe with good friends",
            answerType: "people"
        },
    ]
},
{
    questionNumber: 5,
    questionText: "Where do you spend most time on the internet?",
    answers: [{
            answerNumber: 1,
            answerText: "What's an internet?",
            answerType: "remote"
        },
        {
            answerNumber: 2,
            answerText: "Anywhere with funny cat videos",
            answerType: "wildlife"
        },
        {
            answerNumber: 3,
            answerText: "BBC Good Food",
            answerType: "food"
        },
        {
            answerNumber: 4,
            answerText: "Nowhere, I dropped my phone in to a ravine",
            answerType: "thrill"
        },
        {
            answerNumber: 5,
            answerText: "Twitter",
            answerType: "people"
        },
        {
            answerNumber: 6,
            answerText: "The Guardian Website",
            answerType: "culture"
        },
    ]
},
{
    questionNumber: 6,
    questionText: "Which of these people do you most admire?",
    answers: [{
            answerNumber: 1,
            answerText: "David Attenborough",
            answerType: "wildlife"
        },
        {
            answerNumber: 2,
            answerText: "Graham Norton",
            answerType: "people"
        },
        {
            answerNumber: 3,
            answerText: "Ranulph Fiennes",
            answerType: "remote"
        },
        {
            answerNumber: 4,
            answerText: "Stephen Fry",
            answerType: "culture"
        },
        {
            answerNumber: 5,
            answerText: "Nigella Lawson",
            answerType: "food"
        },
        {
            answerNumber: 6,
            answerText: "Ken Block",
            answerType: "thrill"
        },
    ]
},
{
    questionNumber: 7,
    questionText: "What's your favourite breed of dog?",
    answers: [{
            answerNumber: 1,
            answerText: "Golden Retriever",
            answerType: "people"
        },
        {
            answerNumber: 2,
            answerText: "Labrador",
            answerType: "food"
        },
        {
            answerNumber: 3,
            answerText: "Border Collie",
            answerType: "thrill"
        },
        {
            answerNumber: 4,
            answerText: "Huskie",
            answerType: "remote"
        },
        {
            answerNumber: 5,
            answerText: "King Charles Spaniel",
            answerType: "culture"
        },
        {
            answerNumber: 6,
            answerText: "Rescue Dog",
            answerType: "wildlife"
        },
    ]
},
{
    questionNumber: 8,
    questionText: "Favourite subject at school...",
    answers: [{
            answerNumber: 1,
            answerText: "Geography",
            answerType: "remote"
        },
        {
            answerNumber: 2,
            answerText: "History",
            answerType: "culture"
        },
        {
            answerNumber: 3,
            answerText: "Psychology",
            answerType: "people"
        },
        {
            answerNumber: 4,
            answerText: "Food Tech",
            answerType: "food"
        },
        {
            answerNumber: 5,
            answerText: "Biology",
            answerType: "wildlife"
        },
        {
            answerNumber: 6,
            answerText: "Physical Education",
            answerType: "thrill"
        },
    ]
},
{
    questionNumber: 9,
    questionText: "You're late for work... why?",
    answers: [{
            answerNumber: 1,
            answerText: "I got trapped in a Google blackhole of pointless facts.",
            answerType: "culture"
        },
        {
            answerNumber: 2,
            answerText: "My best friend called to tell me about their date.",
            answerType: "people"
        },
        {
            answerNumber: 3,
            answerText: "I stopped to rescue an upside-down sheep.",
            answerType: "wildlife"
        },
        {
            answerNumber: 4,
            answerText: "I got cut off by 3 feet of snow.",
            answerType: "remote"
        },
        {
            answerNumber: 5,
            answerText: "I got stuck in a glacial cravasse for a few days.",
            answerType: "thrill"
        },
        {
            answerNumber: 6,
            answerText: "I had to go to 3 cafes to find the perfect bircher muesli.",
            answerType: "food"
        },
    ]
},
{
    questionNumber: 10,
    questionText: "What would be your ideal superpower?",
    answers: [{
            answerNumber: 1,
            answerText: "Flight",
            answerType: "thrill"
        },
        {
            answerNumber: 2,
            answerText: "Never putting on weight no matter how much I eat",
            answerType: "food"
        },
        {
            answerNumber: 3,
            answerText: "Infinite knowledge",
            answerType: "culture"
        },
        {
            answerNumber: 4,
            answerText: "Mind reading",
            answerType: "people"
        },
        {
            answerNumber: 5,
            answerText: "The ability to control fire",
            answerType: "remote"
        },
        {
            answerNumber: 6,
            answerText: "Talking to animals",
            answerType: "wildlife"
        },
    ]
},
]

const tieBreaker = [
    {
        personality: "thrill-seeker",
        image: "thrill.jpg"
    },
    {
        personality: "culture-vulture",
        image: "culture.jpg"
    },
    {
        personality: "animal-lover",
        image: "wildlife.jpg"
    },
    {
        personality: "peace-lover",
        image: "remote.jpg"
    },
    {
        personality: "people-person",
        image: "people.jpg"
    },
    {
        personality: "foodie",
        image: "food.jpg"
    }
]